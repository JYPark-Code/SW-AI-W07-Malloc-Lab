...
- [x] 케이스 4-3: 양쪽 블록 free + 합치면 충분 → 양쪽 병합 후 반환
...
- [x] mm_realloc 이전/양쪽 블록 활용 (케이스 4-2, 4-3)

### 시도했으나 효과 없었던 최적화
- asize +4 (header only): ALIGN 특성상 특정 크기에서 +8과 동일 결과
- CHUNKSIZE 2048/8192: throughput 감소 또는 util 대폭 하락
- free 블록 footer 제거: 최소 블록 크기가 24로 유지되어 실익 없음
...
| trace | 파일 | util | 낮은 이유 |
|-------|------|------|----------|
| 4 | coalescing-bal.rep | 66% | ALIGN 특성상 8바이트 낭비 반복 |
| 7 | binary-bal.rep | 60% | 작은/큰 블록 번갈아 할당 → 외부 단편화 |
| 8 | binary2-bal.rep | 53% | binary와 동일 패턴 |
| 9 | realloc-bal.rep | 99% | 이전/다음 블록 병합으로 최적화 |
| 10 | realloc2-bal.rep | 76% | realloc 패턴 최적화 |
...
케이스 4-3: 이전+다음 블록 병합   → remove_free x2 + memmove + 헤더 업데이트
