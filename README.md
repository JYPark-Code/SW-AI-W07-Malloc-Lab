...
| 10 | realloc2-bal.rep | 76% | realloc 패턴 |
...
│ 현재 블록 충분?     → return ptr (그대로)           │
│ 다음 블록 free?     → 병합 후 return ptr            │
│ 이전 블록 free?     → memmove + 병합 후 return      │
│ 양쪽 모두 free?     → memmove + 양쪽 병합 후 return │
│ 모두 해당 없음      → malloc + memcpy + free        │
└─────────────────────────────────────────────────────┘
```
